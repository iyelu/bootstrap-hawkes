function output = HawkesExp_BSmle_res_alp(BSintensity,BSdata,data,StartTime,T,param_res,param_start,optim_options)
% -------------------------------------------------------------------------
...
%   param_res: restricted parameter value (alp_bar this case)
...
% restricted (alp=alp_bar) loglikelihood function with param = [mu, bet]
% see the helper function
objfun = @(param)(-loglik_temp(BSloglikfun,BSdata,data,StartTime,T,alp_bar,param));
...
%% Helper function 
  ...
  function output = loglik_temp(BSloglikfun,BSdata,data,StartTime,T,alp_bar,param)
