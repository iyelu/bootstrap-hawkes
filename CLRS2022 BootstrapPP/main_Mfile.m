...
% This MATLAB script illustrates the asymptotic and boostrap inference 
% of a Hawkes point process discussed in the paper: 
...
if sum(AsymMLE.SanityCheck)==0                % If sanity check is passed
    ...
    for k=1:length(AsymMLE.mle_urs.param)
        fprintf('%8s%8.1f%8.3f%8.3f%5s%5.3f,%5.3f%s\n',...
            param_names{k},param_initial(k),AsymMLE.mle_urs.param(k),AsymMLE.mle_urs.se_hess(k),'[',AsymMLE.CI_hess(k,1),AsymMLE.CI_hess(k,2),']')
    end
    disp(repmat('-',1,60))
else
...
% Below we implement the 4 bootstrap schemes proposed in 
% Cavaliere, Lu, Rahbek et al. (2022) 
% to obtain bootstrap MLE, bootstrap standard errors, and bootstrap percentile intervals.
% 
